...
def get_or_generate_vocab_inner(data_dir, vocab_filename, vocab_size,
    ...
    if data_dir and vocab_filename:
        ...
        if tf.io.gfile.exists(vocab_filepath + '.subwords'):
            tf.compat.v1.logging.info("Found vocab file: %s", vocab_filepath)
            return tfds.features.text.SubwordTextEncoder.load_from_file(vocab_filepath)
    else:
        vocab_filepath = None
    ...
    vocab = tfds.features.text.SubwordTextEncoder.build_from_corpus(
        generator, vocab_size, max_subword_length=max_subtoken_length,
...
def _pack_with_tf_ops(dataset, keys, length):
    ...
    def write_packed_example(partial, outputs):
        ...
        for k in keys_etc:
            new_outputs[k] = outputs[k].write(
                outputs[k].size(),
                tf.pad(tensor=partial[k], paddings=[[0, length - tf.size(input=partial[k])]]))
    ...
    def map_fn(x):
        ...
        dynamic_batch_size = tf.shape(input=x[keys[0]])[0]
        outputs = {}
        for k in keys:
        ...
        def body_fn(i, partial, outputs):
            ...
            for k in keys:
                val = x[k][i]
                val = val[:tf.reduce_sum(input_tensor=tf.cast(tf.not_equal(val, 0), dtype=tf.int32))]
                one_example[k] = val
            for k in keys:
                can_append = tf.logical_and(
                    can_append,
                    tf.less_equal(
                        tf.size(input=partial[k]) + tf.size(input=one_example[k]), length))
            ...
            partial, outputs = tf.cond(pred=can_append, true_fn=true_fn, false_fn=false_fn)
            new_partial = {}
            for k in keys:
                new_seq = one_example[k][:length]
                new_seq_len = tf.size(input=new_seq)
        ...
        i, partial, outputs = tf.while_loop(
            cond=cond_fn, body=body_fn, loop_vars=(i, partial, outputs),
            back_prop=False,
            shape_invariants=(
                tf.TensorShape([]),
        ...
        for k in keys:
            packed[k + "_segmentation"] = (
                    tf.cumsum(tf.cast(tf.equal(packed[k + "_position"], 0), dtype=tf.int32), axis=1) *
                    tf.cast(tf.not_equal(packed[k], 0), dtype=tf.int32))
...
def tfrecord_iterator(filenames, gzipped=False, example_spec=None):
    ...
    with tf.Graph().as_default():
        ...
        def _parse_example(ex_ser):
            return tf.io.parse_single_example(serialized=ex_ser, features=example_spec)
